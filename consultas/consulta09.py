...
def buildC9(frame):
    ...
    ctk.CTkLabel(izq, text="Calles Populares", font=("Arial", 20, "bold"), text_color="white").pack(anchor="w", pady=(0, 20))
    ctk.CTkLabel(izq, text="Calles con mayor número de propiedades en venta.", text_color="#7b7f96").pack(anchor="w", pady=(0, 15))
    ...
    def ejecutar():
        ...
        df['street_name'] = df['street'].str.split(' ', n=1).str[1]
        calles = df['street_name'].value_counts().head(10)
        
        fig, ax = plt.subplots(figsize=(6, 5), facecolor='#1a1d27')
        ax.set_facecolor('#1a1d27')
        calles.plot(kind="bar", color="#f75f5f", ax=ax)
        
        ax.set_title("Top 10 Calles con más propiedades", color="white")
        ax.set_xlabel("Nombre de Calle", color="white")
