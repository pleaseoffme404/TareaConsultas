...
def buildC8(frame):
    ...
    ctk.CTkLabel(izq, text="Tamaño por Tipo", font=("Arial", 20, "bold"), text_color="white").pack(anchor="w", pady=(0, 20))
    ctk.CTkLabel(izq, text="Espacio promedio en pies cuadrados según el tipo.", text_color="#7b7f96").pack(anchor="w", pady=(0, 15))
    ...
    def ejecutar():
        ...
        tamano_tipo = df.groupby("type")["sq__ft"].mean()
        fig, ax = plt.subplots(figsize=(6, 5), facecolor='#1a1d27')
        ax.set_facecolor('#1a1d27')
        tamano_tipo.plot(kind="bar", color="#8b5cf6", ax=ax)
        
        ax.set_title("Tamaño promedio por Tipo", color="white")
        ax.set_xlabel("Tipo de Propiedad", color="white")
        ax.set_ylabel("Pies Cuadrados", color="white")
